...
## 🎯 Актуальные паттерны (версия 1.6.7)
...
### Версия 1.6.7 (текущая)
- ✅ Убран `display: contents` у враппера — flex/grid-контейнеры не ломаются
- ✅ Исправлены ложные срабатывания в CSS/JS-коде — пропуск `STYLE`/`SCRIPT`/`SVG`
- ✅ Удалены отладочные `console.log` из production
- ✅ Оптимизирован MutationObserver (задержка 10 мс)
- ✅ Периодическая проверка 300 мс для чатов и динамического контента
...
3. Скопируйте содержимое `Russian-Uncensored-Words-Highlight-1.6.7.user.js`
...
- `Russian-Uncensored-Words-Highlight-1.6.7.user.js` – пользовательский скрипт для Tampermonkey/Greasemonkey
